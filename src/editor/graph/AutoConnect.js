...
export default 
{
  process(pin, createdNode) {
    ...
    if(pin.type == 'execution' && createdNode.execution) {
      ...
      ActivityManager.history.push(Commander.create('CreateExecution', sourceNode.id, executionName, targetNode.id).processAndSave());
    }
    // If dragging from data pin, connect data pin only
      ...
      if(pin.flow == 'in') {
        ...
        if(firstOutput) {
          SoundEffect.play('link')
          ActivityManager.history.push(Commander.create('CreateDataLink', pin.node.id, pin.name, createdNode.id, firstOutput.name).processAndSave())
        }
      }
      else {
        ...
        if(firstInput) {
          SoundEffect.play('link')
          ActivityManager.history.push(Commander.create('CreateDataLink', createdNode.id, firstInput.name, pin.node.id, pin.name).processAndSave())
        }
      }
    }
  }
