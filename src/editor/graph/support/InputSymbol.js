...
export default class InputSymbol extends DataSymbol
{
  ...
  mouseUp(e) {
    if(this.canConnect(BrainGraph.connectHelper.startSymbol)) {
      ...
      Hub.history.push(Commander.create('CreateDataLink', this.node.id, this.name, 
  ...
  onContextMenu(e) {
    ...
    Hub.history.push(Commander.create('RemoveInputDataLink', this.node.id, this.name).processAndSave());
  }
