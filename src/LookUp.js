...
window.LookUp = {
  ...
  addDataLink: function(entry, id) {
    id = create(entry, id)
    DATALINKS.push(id);
    return id;
  },

  removeDataLink: function(id) {
    let index = DATALINKS.indexOf(id);
    if(index != -1) DATALINKS.splice(index, 1);
    delete STORE[id]
  },
