...
export default class ExecutionOutPin extends ExecutionPin
{
  ...
  draw() {
    ...
    if(adx > ady) {
      this.path.setAttribute('d', `M${this.position.x},${this.position.y} l${ady/2},${dy/2} ${adx-ady},0 ${ady/2},${dy/2} L${this.connectedPin.position.x},${this.connectedPin.position.y}`);
    }
    else {
      // dx with sign of dy
      let dxsdy = adx*Math.sign(dy)
      this.path.setAttribute('d', `M${this.position.x},${this.position.y} l${adx/2},${dxsdy/2}, 0,${(ady-adx)*Math.sign(dy)} ${adx/2},${dxsdy/2} L${this.connectedPin.position.x},${this.connectedPin.position.y}`);
    }
    // else {
    //   if(dy<0) {
    //     this.path.setAttribute('d', `M${this.position.x},${this.position.y} l30,0 0,${dy/2+dx/2} ${dx},${-dx} L${this.connectedPin.position.x-30},${this.connectedPin.position.y} ${this.connectedPin.position.x},${this.connectedPin.position.y}`);
